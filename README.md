
# CodeAlpha UI/UX Internship Tasks

This repository contains my completed tasks for the **CodeAlpha UI/UX Design Internship Program**.

## Intern Details

**Name:** Shahana Beevi
**Internship Domain:** UI/UX Design
**Organization:** CodeAlpha

---

## Completed Tasks

### Task 1 – Wireframing

Created low-fidelity wireframes for an Online Learning Platform named **Skill'IT**.

Screens Included:

* Welcome Screen
* Login Screen
* Sign Up Screen
* Home Screen
* Browse Courses Screen
* Course Details Screen
* My Learning Screen
* Profile Screen


* Wireframe
  ![SkillIT UI](Task_1_wireframe_Skill'IT.png) 
* Figma Design Link
  https://www.figma.com/design/XL1SCrfKz9gMz5TAqfYAH3/Untitled?node-id=0-1&t=kpmWlk6J52MDcqxz-1 

---

### Task 2 – High-Fidelity UI Design

Designed a complete high-fidelity mobile application UI for **Skill'IT**, an online learning platform.

Features:

* Modern and clean interface
* Consistent color palette
* Responsive mobile layout
* User-friendly navigation
* Course discovery and learning flow


* High-Fidelity Design PDF
   ![SkillIT UI](Task_2_Highfidelity_Skill'IT.png)
* Figma Design Link
  https://www.figma.com/design/XL1SCrfKz9gMz5TAqfYAH3/Skill-IT---Online-Learning-Platform?node-id=28-459&t=a5tQmX44Oym49cqK-1 

---

### Task 4 – Interactive Prototype

Developed a fully clickable prototype in Figma demonstrating the complete user journey.

User Flow:
Welcome → Login/Sign Up → Home → Browse Courses → Course Details → My Learning → Profile



* Interactive Prototype Link - https://www.figma.com/proto/XL1SCrfKz9gMz5TAqfYAH3/Skill-IT---Online-Learning-Platform?node-id=28-460&p=f&t=Jy9sRDC0a9pkcGxI-1&scaling=scale-down&content-scaling=fixed&page-id=28%3A459&starting-point-node-id=28%3A460  

* Design Explanation Report - [View Prototype Report](Task_4_Skill'it_REPORT.pdf)

---

## Tools Used

* Figma
* Figma Prototype
* UI/UX Design Principles

---

## Project

### Skill'IT – Online Learning Platform

Skill'IT is a mobile learning application designed to help users discover courses, track learning progress, and build new skills through an intuitive learning experience.

---

## Figma Links


### Wireframes

https://www.figma.com/design/XL1SCrfKz9gMz5TAqfYAH3/Untitled?node-id=0-1&t=kpmWlk6J52MDcqxz-1 

### High-Fidelity Design

https://www.figma.com/design/XL1SCrfKz9gMz5TAqfYAH3/Skill-IT---Online-Learning-Platform?node-id=28-459&t=a5tQmX44Oym49cqK-1 

### Interactive Prototype

https://www.figma.com/proto/XL1SCrfKz9gMz5TAqfYAH3/Skill-IT---Online-Learning-Platform?node-id=28-460&p=f&t=Jy9sRDC0a9pkcGxI-1&scaling=scale-down&content-scaling=fixed&page-id=28%3A459&starting-point-node-id=28%3A460  

---

## Thank You

Thank you to CodeAlpha for providing this opportunity to learn and apply UI/UX design concepts through practical projects.


